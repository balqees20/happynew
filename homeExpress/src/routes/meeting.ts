...
meet_rout.post('/',(req:Request, res:Response) => {
    ...
    connection.query("INSERT INTO meeting (meeting_name,meeting_place,meeting_type ,meeting_date,program_name,meeting_image,description) VALUES('" +M.meeting_name+ "','" +M.meeting_place+ "','" +M.meeting_type+ "','" +M.meeting_date+ "','"+M.program_name+"','"+M.meeting_image+"','"+M.description+"') ", (err, results)=>{
        if (err){
