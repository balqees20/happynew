...
camp_rout.post('/', (req:Request, res:Response) => {
    ...
    connection.query("INSERT INTO camp (camp_name,season,start_date ,last_date,target_group,description,camp_image) VALUES('" +C.camp_name+ "','" +C.season+ "','" +C.start_date+ "','" +C.last_date+ "','"+C.target_group+"','"+C.description+"','"+C.camp_image+"') ", (err, results)=>{
        if (err){
